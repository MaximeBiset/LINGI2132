...
/**
 * The AST node for a plus (+) unary expression
 */
class JUnaryPlusOp extends JUnaryExpression
{
	/**
	 * Construct an AST node for a plus unary expression
	 * given its line number and the arg operand.
	 * 
	 * @params line line in wich the division expression
	 * occurs in the source file.
	 * @params arg arg operand.
	 */
	...
	/**
	 *  Analyzing the + (unary) operation involves analyzing its operand,
	 *  checking type, and determining the result type.
	 *  
	 *  @param context in wich names are resolved.
	 *  @return the analyzed (and possibly rewritten) AST subtree
	 */
	...
	/**
	 * Generating code for the + (unary) operation involves generating code
	 * for his operand, and then then the unary plus instruction.
	 * 
	 * @param output the code emitter (basically an abstraction for
	 * producting the .class file).
	 */
	public void codegen(CLEmitter output)
	{
		arg.codegen(output);
